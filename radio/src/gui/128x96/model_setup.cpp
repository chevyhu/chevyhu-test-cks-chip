 ...
 *Copyright (C) OpenTXCopyright (C) OpenTX
  ...
  for (uint8_t i=0; i<NUM_BODY_LINES; ++i) {
    ...
    switch (k) {
      ...
      case ITEM_MODEL_SETUP_SWITCHES_WARNING:
        ...
        for (int i = 0; i < switchWarningsCount; i++) {
          if (SWITCH_WARNING_ALLOWED(i)) {
            ...
            lcdDrawSizedText(MODEL_SETUP_2ND_COLUMN + qr.rem * ((2 * FW) + 1), y + FH * qr.quot,
                             FIRSTSW_STR + (i * length) + 3, 1,
      ...
      case ITEM_MODEL_SETUP_POTS_WARNING:
        ...
        lcdDrawTextAtIndex(MODEL_SETUP_2ND_COLUMN, y, "\004""OFF\0""Man\0""Auto", g_model.potsWarnMode,
        ...
        if (g_model.potsWarnMode) {
          ...
          for (int i = 0; i < NUM_POTS + NUM_SLIDERS; ++i) {
            if (i < NUM_XPOTS && !IS_POT_SLIDER_AVAILABLE(POT1 + i)) {
              ...
              lcdDrawSizedText(x, y, STR_VSRCRAW + 2 + STR_VSRCRAW[0] * (NUM_STICKS + 1 + i), STR_VSRCRAW[0] - 1,
                               flags & ~ZCHAR);
              x = lcdNextPos + 3;
            }
          }
...
#if 0
        lcdDrawTextAlignedLeft(y, STR_MODE);
        lcdDrawTextAtIndex(MODEL_SETUP_2ND_COLUMN, y, STR_MODULE_PROTOCOLS, g_model.moduleData[EXTERNAL_MODULE].type,
                           menuHorizontalPosition == 0 ? attr : 0);
        if (isModuleXJT(EXTERNAL_MODULE))
          lcdDrawTextAtIndex(lcdNextPos + FW, y, "TEST"/*STR_ACCST_RF_PROTOCOLS*/,
                             1 + g_model.moduleData[EXTERNAL_MODULE].rfProtocol,
                             menuHorizontalPosition == 1 ? attr : 0);
        else if (isModuleDSM2(EXTERNAL_MODULE))
          lcdDrawTextAtIndex(lcdNextPos + FW, y, STR_DSM_PROTOCOLS, g_model.moduleData[EXTERNAL_MODULE].rfProtocol,
                             menuHorizontalPosition == 1 ? attr : 0);
        else if (isModuleR9M(EXTERNAL_MODULE))
          lcdDrawTextAtIndex(lcdNextPos + FW, y, STR_R9M_REGION, g_model.moduleData[EXTERNAL_MODULE].subType,
                             (menuHorizontalPosition == 1 ? attr : 0));
#if defined(MULTIMODULE)
          ...
          switch (menuHorizontalPosition) {
            ...
            case 1:
              if (isModuleDSM2(EXTERNAL_MODULE))
                CHECK_INCDEC_MODELVAR(event, g_model.moduleData[EXTERNAL_MODULE].rfProtocol, DSM2_PROTO_LP45,
                                      DSM2_PROTO_DSMX);
              else if (isModuleR9M(EXTERNAL_MODULE)) {
                uint8_t newR9MType = checkIncDec(event, g_model.moduleData[EXTERNAL_MODULE].subType,
...
#if defined(PCBTARANIS) && defined(BLUETOOTH) && 0
